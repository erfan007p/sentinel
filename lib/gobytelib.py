...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.gobyted_serialise())
...
def SHIM_serialise_for_gobyted(sentinel_hex):
    ...
    # superblock => "trigger" in gobyted
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # gobyted expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    gobyted_hex = serialise(obj)
    return gobyted_hex
